...
## Giai đoạn 1 — PURGE
Starter giữ nguyên **1150µs (`rampfromus`)** trong **10s (`purgestablems`)** đầu tiên
để chạy ổn định trước, **sau đó mới tăng xung +1µs mỗi 250ms (≈ 4µs/s = 2µs/0.5s)**,
tới khi **RPM > 3000** thì **giữ 3s** để thổi sạch khí/dầu dư, rồi sang giai đoạn kế.
Chưa glow, chưa nhiên liệu. Nếu quá `spinuptimeoutms` (55s, đã tính cả 10s ổn định)
mà RPM chưa đạt → abort `NO_RPM`. Áp dụng cho **cả dry test lẫn chạy thật** (dùng
chung hàm ramp).
...
`rampfromus`, `ramptous` (assist max), `rampstepms`, `purgestablems`, `ignarmrpm`, `spinuptimeoutms`,
